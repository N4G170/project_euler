...
std::string Problem052()
{
    ...
    // MessageWriter::Instance()->WriteToOutputBox("P052: "+std::to_string(result)+ " in "+Clock::Instance()->StopAndReturnClock(clock_id) + " ms");
    return "P052: "+std::to_string(result)+ " in "+Clock::Instance()->StopAndReturnClock(clock_id) + " ms";
}
