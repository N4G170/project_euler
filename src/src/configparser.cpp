...
{
    std::ifstream resources_file_stream;
    //choose the specific error flags you whant to check in the stream. If the flags were set before this call, the exception is launched now
    resources_file_stream.exceptions(std::ifstream::badbit | std::ifstream::failbit);

    //try to load default file (if the program got here, no file was provided)
    try
    {
        resources_file_stream.open("data/resource_context.json");

        if(resources_file_stream.is_open())
        {
            resources_file_stream >> resources_json;
        }

        //ALLWAYS close your damn files, although resources_file_stream destructor would close all when its get out of scope
        resources_file_stream.close();
    }
    catch(const std::ios_base::failure &fail)
    {
        //Due to a bug in the GCC compiler, std::ios_base::failure exception are not being thrown as std::ios_base::failure
